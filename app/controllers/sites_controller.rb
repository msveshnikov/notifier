class SitesController < ApplicationController
  ...
  # GET /sites #ALL
  def index
    @sites = Site.where(user_id: current_user ? current_user : User.find(2))
    @categories = Category.select { |cat| cat.sites.where(user_id: current_user ? current_user : User.find(2)).size > 0 }
    @site = Site.new
  end
